...
const ExchangeRatesTable = ({ data, today }: { data: ExchangeRate[], today: string }) => {
    ...
    const loadExchangeByDate = async (date: string) => {
        ...
        try {
            const res = await apiClient.get(`/exchange-rates?start_date=${date}`);
            if (res.status != 200) throw new Error("Failed to fetch exchange rates");

            const result: ExchangeRatesResponse<ExchangeRate> = res;
